...
export default function SWPForm({ schemeCode }) {
  ...
  const calculateSWP = async () => {
    ...
    try {
      ...
      const initialEntry = sortedNAVs.find(nav => new Date(nav.date) >= startDate);
      const initialNAV = initialEntry?.nav;
      ...
      const lastEntry = withdrawalHistory[withdrawalHistory.length - 1] || { remainingValue: 0, remainingUnits: 0, status: '' };
      ...
      setResult({
        ...
        withdrawalHistory,
        totalWithdrawals,
        remainingValue: parseFloat(lastEntry.remainingValue || 0),
        remainingUnits: parseFloat(lastEntry.remainingUnits || 0),
